...
def _step(
    ...
    return offset + scale * np.asarray(a, dtype=np.float64)
...
def main() -> None:
    ...
    scale = np.asarray(robot_cfg.legacy_action_limit, dtype=np.float64)
    ...
    obs_buf = GenLocoObsBuffer(history_length=15, frame_dim=30)
    ...
    if args.viewer:
        ...
        with mujoco.viewer.launch_passive(robot.model, robot.data) as viewer:
            while viewer.is_running():
                ...
                q_des = _step(robot, loader, policy, obs_buf, offset, scale, device, step, step_dt, clip)
                robot.step_substeps(args.decimation, q_des)
                viewer.sync()
                pace_realtime(t0)
                step += 1
