...
- MuJoCo 3.8.0
- PyTorch 2.11.0+cu130（CUDA 13.0，支持 GPU 加速）
- NumPy 2.4.4
...
**说明**：`checkpoint` 内需含名字里带 `actor` 或 `policy` 的线性层权重（与 RSL / Isaac 导出习惯一致）；否则需改 `infer/policy_mlp.py` 里的键过滤逻辑。
...
| 路径                   | 说明                                                                   |
| ---------------------- | ---------------------------------------------------------------------- |
| `infer/`               | MuJoCo：`urdf_to_mjcf`、机器人与 PD、观测缓冲、`.pt` Actor、`run.py`；与训练对齐的常量见 `infer/genloco_a1_cfg.py`，参考运动加载见 `infer/genloco_motion.py` |
| `scripts/`             | `adjust_a1_leg_length.py`：按腿、按段改 URDF 腿长                             |
| `robots/`              | A1 URDF 与 mesh                                                       |
| `GenLoco/`             | 官方 GenLoco 训练仓库（[HybridRobotics/GenLoco](https://github.com/HybridRobotics/GenLoco)） |
